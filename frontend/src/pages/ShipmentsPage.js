...
export default function ShipmentsPage() {
  ...
  const calculateMetrekare = () => {
    ...
    if (urun_tipi === 'Normal Ürün (Metre)') {
  ...
  const handleSubmit = async (e) => {
    ...
    const submitData = {
      ...
      metre: formData.urun_tipi === 'Normal Ürün (Metre)' ? parseFloat(formData.metre) : null,
    ...
    try {
      if (editMode) {
        ...
        toast.success('Yeni sevkiyat kaydı eklendi');
      }
      fetchShipments();
      setDialogOpen(false);
      resetForm();
  ...
  const filteredShipments = shipments.filter((item) => {
    ...
    return (
      item.musteri_adi?.toLowerCase().includes(search) ||
      item.irsaliye_no?.toLowerCase().includes(search) ||
      item.plaka?.toLowerCase().includes(search) ||
  ...
  return (
    <div className=\"space-y-6\">
      <div className=\"flex items-center justify-between\">
        ...
        >
          ...
          <DialogContent className=\"max-w-2xl max-h-[90vh] overflow-y-auto\">
            <DialogHeader>
              <DialogTitle>{editMode ? 'Sevkiyat Düzenle' : 'Yeni Sevkiyat Kaydı'}</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className=\"space-y-4\">
              <div className=\"grid grid-cols-2 gap-4\">
                <div>
                  ...
                  <Input
                    type=\"date\"
                    value={formData.sevkiyat_tarihi}
                    onChange={(e) =>
                      setFormData({ ...formData, sevkiyat_tarihi: e.target.value })
                    }
                    required
                  />
                </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({ ...formData, urun_tipi: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value=\"Normal Ürün (Metre)\">Normal Ürün (Metre)</SelectItem>
              ...
              {formData.urun_tipi === 'Normal Ürün (Metre)' ? (
                <div>
                  <Label>Metre (m)</Label>
                  <Input
                    type=\"number\"
                    ...
                    onChange={(e) => setFormData({ ...formData, metre: e.target.value })}
                    required
                  />
                </div>
              ) : (
      ...
      <div className=\"grid grid-cols-3 gap-4\">
        <Card>
          <CardHeader className=\"pb-2\">
            <CardTitle className=\"text-sm\">Toplam Sevkiyat</CardTitle>
          </CardHeader>
          <CardContent>
            <p className=\"text-2xl font-bold\">{totalSevkiyat}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className=\"pb-2\">
      ...
      <div>
        <Input
          placeholder=\"Ara (Müşteri, İrsaliye No, Plaka, Renk)...\"
          value={searchTerm}
              ...
              <TableBody>
                {filteredShipments.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>{item.sevkiyat_tarihi}</TableCell>
                    <TableCell>{item.musteri_adi}</TableCell>
                    <TableCell>
                      <span className=\"text-xs\">{item.urun_tipi}</span>
                    </TableCell>
                    <TableCell>{item.kalinlik}</TableCell>
                    <TableCell>{item.en}</TableCell>
                    <TableCell>{item.metre || item.boy || '-'}</TableCell>
