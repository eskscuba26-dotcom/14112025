...
export default function ProductionPage() {
  ...
  const calculateGazAgirligi = () => {
    const { kalinlik, en, boy, adet } = formData;
    if (kalinlik && en && boy && adet) {
      return ((parseFloat(kalinlik) * parseFloat(en) * parseFloat(boy) * parseInt(adet) * 0.92) / 1000000).toFixed(2);
    }
    return '0.00';
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        toast.success('Yeni kayit eklendi');
      }
      fetchProductions();
      setDialogOpen(false);
      resetForm();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Uretim Girisi</h1>
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editMode ? 'Kayit Duzenle' : 'Yeni Uretim Kaydi'}</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Tarih</Label><Input type="date" value={formData.tarih} onChange={(e) => setFormData({ ...formData, tarih: e.target.value })} required /></div>
                <div><Label>Makine</Label><Select value={formData.makine} onValueChange={(value) => setFormData({ ...formData, makine: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="MAKINE 1">MAKINE 1</SelectItem><SelectItem value="MAKINE 2">MAKINE 2</SelectItem><SelectItem value="MAKINE 3">MAKINE 3</SelectItem></SelectContent></Select></div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div><Label>Gaz Agirligi (kg)</Label><Input value={calculateGazAgirligi()} disabled /></div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div><Label>Masura</Label><Input type="number" value={formData.masura} onChange={(e) => setFormData({ ...formData, masura: e.target.value })} /></div>
                <div><Label>M.Adet</Label><Input type="number" value={formData.m_adet} onChange={(e) => setFormData({ ...formData, m_adet: e.target.value })} /></div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Toplam Uretim</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{totalAdet} Adet</p></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Toplam Metrekare</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{totalMetrekare.toFixed(2)} m2</p></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Toplam Gaz Agirligi</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{totalGaz.toFixed(2)} kg</p></CardContent></Card>
      </div>
      <div><Input placeholder="Ara (Makine, Model, Renk)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      <Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Tarih</TableHead><TableHead>Makine</TableHead><TableHead>Kalinlik</TableHead><TableHead>En</TableHead><TableHead>Boy</TableHead><TableHead>Adet</TableHead><TableHead>m2</TableHead><TableHead>Masura</TableHead><TableHead>M.Adet</TableHead><TableHead>Model</TableHead><TableHead>Gaz (kg)</TableHead><TableHead>Renk</TableHead><TableHead>Islemler</TableHead></TableRow></TableHeader><TableBody>{filteredProductions.map((item) => (<TableRow key={item.id}><TableCell>{item.tarih}</TableCell><TableCell>{item.makine}</TableCell><TableCell>{item.kalinlik}</TableCell><TableCell>{item.en}</TableCell><TableCell>{item.boy}</TableCell><TableCell>{item.adet}</TableCell><TableCell>{item.metrekare.toFixed(2)}</TableCell><TableCell>{item.masura || '-'}</TableCell><TableCell>{item.m_adet || '-'}</TableCell><TableCell>{item.model || '-'}</TableCell><TableCell>{item.gaz_agirligi.toFixed(2)}</TableCell><TableCell>{item.renk && <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs font-medium">{item.renk}</span>}</TableCell><TableCell><div className="flex space-x-2"><Button size="sm" variant="outline" onClick={() => handleEdit(item)}>Duzenle</Button><Button size="sm" variant="destructive" onClick={() => handleDelete(item.id)}>Sil</Button></div></TableCell></TableRow>))}</TableBody></Table></div></CardContent></Card>
    </div>
  );
}
