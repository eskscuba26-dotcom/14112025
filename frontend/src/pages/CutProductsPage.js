...
export default function CutProductsPage() {
  ...
  const fetchData = async () => {
    try {
      const [cutRes, prodRes] = await Promise.all([axios.get(`${API}/cut-products`), axios.get(`${API}/production`)]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        toast.success('Yeni kesim kaydi eklendi');
      }
      fetchData();
      setDialogOpen(false);
      resetForm();
  ...
  const filteredCutProducts = cutProducts.filter((item) => {
    ...
    return item.boyut?.toLowerCase().includes(search) || item.durum?.toLowerCase().includes(search) || item.renk?.toLowerCase().includes(search);
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Kesilmis Urun</h1>
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild><Button>Yeni Kesim</Button></DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editMode ? 'Kesim Duzenle' : 'Yeni Kesim Kaydi'}</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div><Label>Ana Urun Sec</Label><Select value={formData.ana_urun_id} onValueChange={(value) => { const prod = productions.find((p) => p.id === value); setFormData({ ...formData, ana_urun_id: value, kalinlik: prod?.kalinlik.toString() || '', en: prod?.en.toString() || '', boy: prod?.boy.toString() || '', renk: prod?.renk || '' }); }}><SelectTrigger><SelectValue placeholder="Ana urun seciniz" /></SelectTrigger><SelectContent>{productions.map((prod) => (<SelectItem key={prod.id} value={prod.id}>{prod.kalinlik}x{prod.en}x{prod.boy} - {prod.makine} ({prod.adet} adet){prod.renk && ` - ${prod.renk}`}</SelectItem>))}</SelectContent></Select></div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div><Label>Ihtiyac (Adet)</Label><Input type="number" value={formData.adet} onChange={(e) => setFormData({ ...formData, adet: e.target.value })} required /></div>
                <div><Label>Kullanilan (Adet)</Label><Input type="number" value={formData.kullanilan} onChange={(e) => setFormData({ ...formData, kullanilan: e.target.value })} required /></div>
                <div><Label>m2 (Otomatik)</Label><Input value={calculateMetrekare()} disabled /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Kesim Tarihi</Label><Input type="date" value={formData.tarih} onChange={(e) => setFormData({ ...formData, tarih: e.target.value })} required /></div>
                <div><Label>Kalite Durumu</Label><Select value={formData.durum} onValueChange={(value) => setFormData({ ...formData, durum: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Onayli">Onayli</SelectItem><SelectItem value="Beklemede">Beklemede</SelectItem><SelectItem value="Reddedildi">Reddedildi</SelectItem></SelectContent></Select></div>
              </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Toplam Kesim</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{totalKesim}</p></CardContent></Card>
      ...
      <div><Input placeholder="Ara (Boyut, Durum, Renk)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      <Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Boyut</TableHead><TableHead>Adet</TableHead><TableHead>m2</TableHead><TableHead>Ana Urun</TableHead><TableHead>Kullanilan</TableHead><TableHead>Tarih</TableHead><TableHead>Durum</TableHead><TableHead>Renk</TableHead><TableHead>Islemler</TableHead></TableRow></TableHeader><TableBody>{filteredCutProducts.map((item) => (<TableRow key={item.id}><TableCell>{item.boyut}</TableCell><TableCell>{item.adet}</TableCell><TableCell>{item.metrekare.toFixed(2)}</TableCell><TableCell>{item.ana_urun_id.substring(0, 8)}...</TableCell><TableCell>{item.kullanilan}</TableCell><TableCell>{item.tarih}</TableCell><TableCell><span className={`px-2 py-1 rounded text-xs font-medium ${item.durum === 'Onayli' ? 'bg-green-100 text-green-800' : item.durum === 'Beklemede' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{item.durum}</span></TableCell><TableCell>{item.renk && <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded text-xs font-medium">{item.renk}</span>}</TableCell><TableCell><div className="flex space-x-2"><Button size="sm" variant="outline" onClick={() => handleEdit(item)}>Duzenle</Button><Button size="sm" variant="destructive" onClick={() => handleDelete(item.id)}>Sil</Button></div></TableCell></TableRow>))}</TableBody></Table></div></CardContent></Card>
    </div>
  );
}
